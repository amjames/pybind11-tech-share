...
def build():
    ...
    valid_metadata_keys = set(['title', 'use_katex', 'number_slides', 'language'])
    metadata = {}
    for line in slides:
    ...
    if len(metadata) != len(valid_metadata_keys):
        raise ValueError("Be sure to include title: language, number_slides, and use_katex as metadata "
                         "in slides.md file")

    output = template.render(title=metadata['title'],
                             use_katex=metadata['use_katex'] == 'True',
                             language=metadata['language'],
                             number_slides=metadata['number_slides'],
                             slides="".join(slides))
